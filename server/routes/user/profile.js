...
// ROUTE 1: get user (admin) profile details
...
// ROUTE 5: get a user by its userId
router.get("/find", fetchUser, async (req, res) => {
  try {
    ...
    const data = await User.findById(searchTerm);
    if (!data)
      return res.status(404).json({
        error: {
          code: 404,
    ...
    console.log("Error in all find profile route: ", error);
    res.status(500).json({
      error: {
        code: 500,
        message: "Internal Server Error Occurred! Try again later",
